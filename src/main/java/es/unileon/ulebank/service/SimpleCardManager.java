...
/**
 * @author Israel Funciona como mediador entre el controlador y la clase que
...
public class SimpleCardManager implements CardManager {
    ...
    public List<Card> getCards(Handler dni) {
        return this.cardDao.getCards(dni);
    }
    ...
    public void changeBuyLimits(double diary, double monthly, Handler handler)
            throws Exception {
        final Card card = this.cardDao.findById(handler);
        final Command buyLimitsDiary = new ModifyBuyLimitCommand(card.getId(),
                card, diary, "diary");
        final Command buyLimitsMonthly = new ModifyBuyLimitCommand(
                card.getId(), card, monthly, "monthly");
    ...
    public void changeCashLimits(double diary, double monthly, Handler handler)
            throws Exception {
        final Card card = this.cardDao.findById(handler);
        final Command cashLimitsDiary = new ModifyCashLimitCommand(
                card.getId(), card, diary, "diary");
        final Command cashLimitsMonthly = new ModifyCashLimitCommand(
                card.getId(), card, monthly, "monthly");
