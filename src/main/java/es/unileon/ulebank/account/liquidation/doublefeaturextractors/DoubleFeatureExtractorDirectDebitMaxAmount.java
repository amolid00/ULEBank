...
AbstractFeatureExtractor<Double> {
    ...
    private static final Logger LOG = Logger
            .getLogger(DoubleFeatureExtractorDirectDebitMaxAmount.class);
    ...
    public void updateFeature(Account account, Date min, Date max) {
        ...
        try {
            ...
            DoubleFeatureExtractorDirectDebitMaxAmount.LOG.error(e);
        }
        for (final DirectDebitTransaction actual : list) {
            if ((actual.getAmount() < 0) && (actual.getAmount() < maxAmount)) {
